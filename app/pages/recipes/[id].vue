...
<template>
  <div class="flex flex-col max-w-screen-lg container mx-auto py-20 px-4">
    <!-- Header -->
    <div class="flex flex-col mb-6">
      <h2 class="text-3xl mb-4 font-semibold">{{ data?.name }}</h2>
      <div class="flex gap-4 text-xl mb-6">
        ...
        <div class="flex items-center gap-1">
          <UIcon name="i-mdi-fire" class="text-yellow-500" />
        ...
        <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
          <li v-for="ingredient in data?.ingredients">
            <label class="flex gap-2 items-center cursor-pointer select-none">
              ...
              <!-- Custom circle -->
              <div
                class="relative w-6 h-6 rounded-full border-2 border-yellow-500 flex items-center justify-center after:content-[''] after:rounded-full peer-checked:after:absolute peer-checked:after:w-3 peer-checked:after:h-3 peer-checked:after:bg-yellow-500"
              ></div>

              <!-- Text with strikethrough on checked -->
    ...
    <div>
      ...
      <ul class="flex flex-col text-lg gap-4">
        ...
        >
          <span
            class="flex items-center justify-center bg-yellow-500 w-7 h-7 rounded-full text-white text-sm"
          >
            {{ index + 1 }}
          </span>
